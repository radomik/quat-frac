...
/// @see readNewByText
void OptDrawingCreator::_readJsonCommon(const nlohmann::json &json, const char *tag)
{
...
void OptDrawingCreator::readJson(const nlohmann::json &json, const char *tag)
{
...
/// @note version is already read
/// @see readNewByBin()
void OptDrawingCreator::_readBinCommon(FILE *file, const char *tag)
{
...
// read common + custom fields
void OptDrawingCreator::readBin(FILE *file, const char *tag)
{
...
nlohmann::json OptDrawingCreator::_saveJsonCommon(const char *tag) const {
  U32     val_uint;
  YesNo o_color_negative;
...
nlohmann::json OptDrawingCreator::saveJson(const char *tag) const
{
  // save fields common to all classes derived from OptDrawingCreator
  if (_saveTextCommon(tag) < 0) return -1;

  // save child class fields
  return customSaveText(tag);
}

/// save common fields (also version)
void OptDrawingCreator::_saveBinCommon(FILE *file, const char *tag) const
{
...
// save common + custom fields
void OptDrawingCreator::saveBin(FILE *file, const char *tag) const {
  // save fields common to all classes derived from OptDrawingCreator
...
OptDrawingCreator* OptDrawingCreator::readNewByJson(const nlohmann::json &json, const char *tag) {
  const char *self_name = "OptDrawingCreator::readText";
...
// Create new custom object with read all fields
OptDrawingCreator* OptDrawingCreator::readNewByBin(FILE *file, const char *tag) {
...
OptDrawingCreator* OptDrawingCreator::_st_readText(const nlohmann::json &json, const char *tag, unsigned int version) {
  OptDrawingCreator* draw = newByVersion(version);
  if (! draw) return NULL;
